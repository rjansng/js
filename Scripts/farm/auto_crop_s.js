...
if (蝦蝦果園KEY == '') { 蝦蝦果園KEY = 'f=1=9w3r/u//f+//Rawym8a5Fwj4gZWT'; $persistentStore.write(蝦蝦果園KEY, '蝦蝦果園KEY'); }
...
const caption = '自動種植 次要';
const title = '蝦蝦果園 ' + caption;
const version = 'v20240111';
...
async function ProcData2(data, dc) {
	return new Promise((resolve, reject) => {
		...
		if (json.code == 0) {
			if (json.msg === 'success') {
				found = true;
				console.log(`${config.metaName} 種植成功。`);
				...
				try {
					...
					s.c++;
					s.f = true;
					s.r += `\n🌱${config.metaName} : ${(new Date()).format('3')}`;
					ts[tsid] = s;
					tasks[tsn] = ts;
		...
		} else if (json.code == 409009) { //,"msg":"crop is waiting"
			console.log('種植失敗，尚未開放種植，還不能種植。');
		}
		else {
			return reject([`執行失敗 ‼️`, json.msg, data]);
		}
...
['POST', '種植作物', '2', 'https://games.shopee.tw/farm/api/orchard/crop/create', '', , ProcData2],
];
...
function preInit() {
	...
	config.hasCrop = false;
	config.metaId = parseInt($persistentStore.read('蝦蝦果園待種植作物ID_S') || '0');
	config.metaName = $persistentStore.read('蝦蝦果園待種植作物名稱_S') || '';
	DataPostBodyList[2].metaId = config.metaId;
	if (config.metaId != 0) { config.hasCrop = true; }
}
...
(async () => {
	...
	try {
		...
		let runCount = 0;
		let item = -1;
		console.log(`蝦蝦果園待種植作物_S: ${config.metaId} ${config.metaName}`);
		...
		for (let i = 1; i < UrlData.length; i++) {
			...
			item = -1;
			if (i == 2) {
				if (config.canCreate && config.hasCrop) {
					if (蝦蝦果園KEY != '') {
						let autoCropSecond = parseFloat($persistentStore.read('自動種植等待秒') || '0.0');
						if (autoCropSecond > 0) { await delay(autoCropSecond); }
						DataPostBodyList[i].iframe_s = EncryptCBC("".concat(Date.now(), "-").concat(config.SPC_U));
					}
					else {
						console.log('沒有「蝦蝦果園KEY」，無法種植。');
						flag = false; break;
					}
				} else {
					console.log('目前無法種植。');
