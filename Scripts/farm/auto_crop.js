...
function preInit() {
	...
	config.metaId = parseInt($persistentStore.read('蝦蝦果園待種植作物ID') || '0');
	config.metaName = $persistentStore.read('蝦蝦果園待種植作物名稱') || '';
...
(async () => {
	...
	try {
		...
		let runCount = 0;
		let item = -1;
		console.log(`蝦蝦果園待種植作物: ${config.metaId} ${config.metaName}`);
