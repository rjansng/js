...
// @version           3.3.6
// @description       【❤️ 视频自动解析，体会拥有VIP的感觉❤️ 】功能有：1、解锁B站大会员番剧、B站视频解析下载；2、爱奇艺、腾讯、优酷、芒果等全网VIP视频免费破解去广告(免跳出观影特方便【PC端+移动端】)；3、网易云音乐、QQ音乐、酷狗、蜻蜓FM、荔枝FM、喜马拉雅等音乐和有声书音频免客户端下载；4、油管、Facebook等国外视频解析下载；5、网盘搜索引擎(来搜一下 www.laisoyixia.com)破解无限下载；6、优惠券查询等；7、搜索引擎导航【脚本长期维护更新，完全免费，无广告】
// @author            爱画画的猫
...
(function () {
	...
	function superVideoHelper(originalInterfaceList, playerNodes){
		...
		this.removeVideoAdBlock_iqiyi = function(){
			var result = false;
			Array.from(document.getElementsByClassName('iqp-integral-inner')).forEach(function (element){element.remove();});
			setInterval(function (){
				try{					
					result = false;
					if (!result && document.getElementsByClassName('cupid-public-time')[0] 
							&& document.getElementsByClassName('cupid-public-time')[0].style.display.length==0) {
						document.querySelector('#flashbox > iqpdiv > iqpdiv.iqp-player > video').removeAttribute('src');
						document.querySelector('#flashbox > iqpdiv > iqpdiv.iqp-player > video').load();
						result = true;
						commonFunctionObject.webToast({"message":"【去广告中】", "background":"#FFFFFF"});
					}
					if (result && document.getElementsByClassName('skippable-after')[0]) {
						document.getElementsByClassName('skippable-after')[0].click();
						result = false;
					}
					if(document.getElementsByClassName('cupid-pause-close')[0]){
						document.getElementsByClassName('cupid-pause-close')[0].parentNode.remove();
					}
					Array.from(document.getElementsByClassName('iqp-logo-top logoShowAnimation'))
						.forEach(function (element){element.remove();});
					Array.from(document.getElementsByClassName('iqp-logo-box'))
						.forEach(function (element){element.remove();});
					Array.from(document.getElementsByClassName('iqp-logo-bottom logoShowAnimation'))
						.forEach(function (element){element.remove();});
				}catch(e){}
			}, 110);
		},
				...
				if(resutData.result==="success" && !!resutData.json){
					...
					if($llkk.length != 0){
						...
						couponElementA.unbind("click").bind("click", ()=>{
							...
							if(!!couponId){
								console.log("goodsPrivateUrl+couponId",goodsPrivateUrl+couponId);
		...
		this.getNavigationData = function(element, elementInput){
			let navigationData = [
				{"name":"资源搜索","list":[
					...
					{"name":"搜狗|知乎", "url":"https://www.sogou.com/sogou?pid=sogou-wsse-ff111e4a5406ed40&insite=zhihu.com&ie=utf8&p=73351201&query=ds&ie=utf8&p=73351201&query=@@"},
					...
					{"name":"PPT搜索", "url":"https://www.ypppt.com/p/search.php?kwtype=1&q=@@"},
			...
			this.createHtml(element, elementInput, navigationData);
		};
		...
		this.show = function(){
			...
			if((host==="www.baidu.com")
				...
				if(currentSearchEnginesData!=null){
					...
					console.log(currentSearchEnginesData);
				}
			}
		};
		this.start = function(){
